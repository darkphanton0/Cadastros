...
def verificar_atualizacao():
    root_temp = tk.Tk()
    root_temp.withdraw()  # janela invisível só para messagebox
...
def pesquisar_produto():
    ...
    for i, p in enumerate(produtos):
        if (not ncm or p.get("ncm","").startswith(ncm)) and \
           (not desc or desc in p.get("descricao","").upper()) and \
           (not pis or p.get("pis_cofins","") == pis) and \
           (not icms or p.get("icms","") == icms):
...
def abrir_formulario(produto=None, indice=None):
    ...
    tk.Label(janela, text="Natureza Receita:").pack(pady=5)
